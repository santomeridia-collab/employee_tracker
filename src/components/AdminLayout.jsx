...
const AdminLayout = ({ children }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      await signOut(auth);
      // Navigate to login (or simply navigate if hardcoded admin)
      navigate('/');
    } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-slate-900">
      {/* --- Sidebar --- */}
      <div className="w-64 flex-shrink-0 bg-white dark:bg-slate-800 shadow-xl transition-all duration-300 overflow-y-auto">
        <div className="p-6 border-b border-gray-200 dark:border-slate-700">
          <h1 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">Santo Meridia</h1>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex-shrink-0 flex justify-between items-center h-16 bg-white dark:bg-slate-800 shadow-md p-4 sticky top-0 z-10">
          ...
          <button
            ...
            className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm font-medium transition duration-150"
          >
            Logout
          </button>
        </header>
